const audienceRows = [
  { y: 304, xs: [112, 160, 208, 256, 304] },
  { y: 354, xs: [88, 140, 192, 244, 296, 348] },
  { y: 408, xs: [64, 120, 176, 232, 288, 344, 400] },
] as const;
...
export default function PipelineVisual() {
  return (
    <svg
      className="data-visual pipeline-visual"
      viewBox="0 0 760 560"
      role="img"
      aria-label="Семинар формирует партнёрскую сеть и новый прибыльный бизнес-юнит"
    >
      <defs>
        <linearGradient id="seminarGlass" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0" stopColor="#17394a" stopOpacity=".82" />
          <stop offset="1" stopColor="#04111a" stopOpacity=".92" />
        </linearGradient>
        ...
        <radialGradient id="coinFace" cx="38%" cy="28%" r="74%">
          ...
          <stop offset="1" stopColor="#061d29" stopOpacity=".96" />
        </radialGradient>
      ...
      <g className="seminar-grid">
        {Array.from({ length: 9 }).map((_, index) => (
          <line key={`v-${index}`} x1={44 + index * 84} y1="52" x2={44 + index * 84} y2="512" />
        ))}
        {Array.from({ length: 6 }).map((_, index) => (
          <line key={`h-${index}`} x1="40" y1={66 + index * 84} x2="720" y2={66 + index * 84} />
        ))}
      </g>
      ...
      <g className="seminar-audience">
        {audienceRows.flatMap((row, rowIndex) => row.xs.map((x, index) => (
          <g
            className="audience-person"
            key={`${row.y}-${x}`}
            style={{ animationDelay: `${(rowIndex * 6 + index) * 90}ms` }}
      ...
      <g className="seminar-signals">
        {signalDots.map(([x, y], index) => (
          <circle
            className="seminar-signal"
            key={`${x}-${y}`}
            cx={x}
            cy={y}
            r={index % 2 === 0 ? 5 : 3.5}
      ...
      <g className="partner-network">
        ...
        {networkNodes.map(([x, y], index) => (
          <g className="network-node" key={`${x}-${y}`} style={{ animationDelay: `${index * 180}ms` }}>
      ...
      <path className="business-flow" d="M618 296 C652 326 660 354 654 374" />
      <g className="business-coin" transform="translate(654 430)">
